...
// src/api-handlers/danbooru.ts
var getRandomPost = async () => {
  const baseTag = Bun.env.MAIN_TAG;
...
// src/routes/test/danbooru.ts
var danbooruRoute = new Hono2;
danbooruRoute.get("/danbooru", async (c) => {
  const post = await getRandomPost();
  return c.json({ post });
});
var danbooru_default = danbooruRoute;
...
// src/helpers/lineServices.ts
var sendLineMessage = async (replyToken, message) => {
...
var sendFlexMessage = async (replyToken, flexMessage) => {
  try {
    const url = `https://api.line.me/v2/bot/message/reply`;
    const headers = {
      "Content-Type": "application/json",
...
var sendImageToLineViaReplyToken = async (replyToken, post, characterTag, characterName) => {
  try {
    const url = `https://api.line.me/v2/bot/message/reply`;
    const headers = {
      "Content-Type": "application/json",
...
Large Img: ${post.large_file_url}`
    ...
    const messages = [imageMessage, textMessage];
    if (characterTag) {
      const flexMessage = {
        type: "flex",
...
var rerollFlexMessage = (characterTag, characterName, artist) => {
...
var quotaFlexMessage = (used, total) => {
  return {
    type: "carousel",
    contents: [
      {
...
var helpFlexMessage = (characterList) => {
...
// src/helpers/danbooruServices.ts
var getRandomPost2 = async () => {
  const baseTag = Bun.env.MAIN_TAG;
  const ratingFilter = "rating:s";
  if (!baseTag) {
    console.error("MAIN_TAG is undefined");
...
var getPostByCharacterTag = async (tag) => {
  const ratingFilter = "rating:s";
...
// src/helpers/geminiServices.ts
var getGeminiResponse = async (message) => {
  try {
...
var handleLineWebhook = async (c) => {
  ...
  for (const event of body.events) {
    ...
    if (type === "message") {
      ...
      if (text.includes("/rd")) {
        const post = await getRandomPost2();
      ...
      if (text.includes("@mkbot")) {
        ...
        const message2 = text.replace("@mkbot", "").trim();
        if (!message2) {
          await sendLineMessage(replyToken, "Please provide a message after @mkbot");
          return;
        }
        const response = await getGeminiResponse(message2);
...
// src/routes/trigger/line.ts
var lineRoute = new Hono2;
lineRoute.post("/line", async (c) => {
  return handleLineWebhook(c);
});
var line_default = lineRoute;

// src/index.ts
var app = new Hono2;
app.use("*", logger());
app.get("/", (c) => c.json({ message: "running" }));
app.route("/test", danbooru_default);
app.route("/api", line_default);
var src_default = app;
export {
